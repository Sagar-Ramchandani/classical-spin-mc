...
function plotHist(axis, mPlanar::Matrix{Float64}, bins::Int64, plotrange::NTuple{2,Vector{Float64}})
...
function plotHistMultiple(fn::String, bins::Int64; saveLocation::Union{String,Nothing}=nothing, kwargs...)
    ...
    for i in eachindex(files)
        ...
        !(saveLocation === nothing) && save(saveLocation * "/hist2d$(i).pdf", axis.scene, pt_per_unit=246 / 600)
        push!(figs, fig)
        push!(axises, figs)
        println(i)
    end
