function plotProperty!(ax, y; x = nothing, color = defaultColor, kwargs...)
    ...
    ax.errorbar(xAxis, yAxis, Δy, color = color,
        zorder = defaultZOrder, ecolor = errorColor; kwargs...)
    ax.scatter(xAxis, yAxis, marker = defaultMarker, s = defaultMarkerSize,
        color = color, zorder = defaultZOrder + 1; kwargs...)
end
...
function setTicks(ax, l::Rational, u::Rational)
    ax.set_yticks(
        (l, u), labels = [i.den == 1 ? "$(i.num)" : "$(i.num)/$(i.den)" for i in (l, u)])
end
    ...
    for (i, (ax, prop)) in enumerate(zip(axis, properties))
        ...
        if markLines && !(markLine === nothing)
            if length(markLine) == 2
                ax.plot(
                    ax.get_xlim(), markLine, c = transitionColor, zorder = defaultZOrder)
            else
                ax.plot(ax.get_xlim(), (markLine, markLine),
                    c = transitionColor, zorder = defaultZOrder)
            end
        end
