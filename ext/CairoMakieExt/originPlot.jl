...
function plotArrow(ax, startPoint, endPoint; lengthRatio=1, color=defaultColor, kwargs...)
    δ = endPoint - startPoint
    arrows!(ax, [[i] for i in startPoint]..., [[i] for i in δ]..., color=color, arrowsize=[0.1, 0.1, 0.3])
end

function scatterVertices3D!(ax, points; color=defaultColor, markersize=5,distanceShade=false, kwargs...)
    ...
    scatter!(ax, getindex.(points, 1), getindex.(points, 2), getindex.(points, 3), markersize=markersize, color=color; kwargs...)
end
...
function plotSphere!(axis, resolution; color=[sphereColor], kwargs...)
    ...
    y = sin.(u) * sin.(v)'
    z = ones(N) * cos.(v)'
    surface!(axis, x, y, z, colormap=color, shading=NoShading, alpha=0.1, rasterize=true; kwargs...)
...
function copBase(; arrowL=0.0, limL=1, init=(acos(1 / sqrt(3)), π / 4))
    ...
    axis = Axis3(fig[1, 1], aspect=:equal, elevation=init[1], azimuth=init[2], protrusions=0, viewmode=:fit)
...
function originPlot!(axis, fn; kwargs...)
    ...
    map((x) -> plotSpins!(axis, x[1], color=x[2]; kwargs...), zip(groupedSpins, XYZColors))
end
...
function originPlotMultiple(location; saveLocation=nothing)
    ...
    for i in eachindex(filesPerFolder)
        ...
        push!(figs, fig)
        push!(axises, figs)
        !(saveLocation === nothing) && save(saveLocation * "/cop$(i).pdf", axis.scene, pt_per_unit=246 / 600)
        println(i)
    end
    return figs, axises
end
