...
@kwdef mutable struct UnitCell{D}
    ...
    interactions::Vector{Tuple{Pair{Int, Int}, NTuple{D, Int}, SMatrix{3, 3, Float64, 9}}} = Vector{Tuple{
        Pair{Int, Int}, NTuple{D, Int}, SMatrix{3, 3, Float64, 9}}}(undef, 0)
    interactionsOnsite::Vector{SMatrix{3, 3, Float64, 9}} = Vector{SMatrix{
        3, 3, Float64, 9}}(undef, 0)
...
"""
function addAnisotropy!(unitcell::UnitCell{D}, func::F,
        funcParameters::Vector{Float64}) where {D, F <: Function}
...
function addInteraction!(
        unitcell::UnitCell{D}, b::Pair{Int, Int}, M::SMatrix{3, 3, Float64, 9},
        offset::NTuple{D, Int} = Tuple(zeros(Int, D))) where {D}
...
"""
function addInteraction!(
        unitcell::UnitCell{D}, b1::Int, b2::Int, M::SMatrix{3, 3, Float64, 9},
        offset::NTuple{D, Int} = Tuple(zeros(Int, D))) where {D}
...
"""
function addInteraction!(unitcell::UnitCell{D}, b1::Int, b2s::Vector{Int},
        M::SMatrix{3, 3, Float64, 9}, offsets::Vector{NTuple{D, Int}}) where {D}
...
"""
function addInteraction!(unitcell::UnitCell{D}, b1::Int, b2::Int, M::Matrix{Float64},
        offset::NTuple{D, Int} = Tuple(zeros(Int, D))) where {D}
...
"""
function setInteractionOnsite!(
        unitcell::UnitCell{D}, b::Int, M::SMatrix{3, 3, Float64, 9}) where {D}
...
"""
function setInteractionOnsite!(
        unitcell::UnitCell{D}, M::Vector{SMatrix{3, 3, Float64, 9}}) where {D}
...
"""
function addBasisSite!(
        unitcell::UnitCell{D}, positions::Vector{SVector{D, Float64}}) where {D}
...
"""
function addBasisSite!(
        unitcell::UnitCell{D}, positions::Vector{NTuple{D, Float64}}) where {D}
    for position in positions
        addBasisSite!(unitcell, SVector(position))
    end
    return nothing
...
function resetBasis!(unitcell::UnitCell{D}) where {D}
    ...
    unitcell.interactions = Vector{Tuple{
        Pair{Int, Int}, NTuple{D, Int}, SMatrix{3, 3, Float64, 9}}}(undef, 0)
...
function Base.:show(io::IO, uc::UnitCell{D}) where {D}
    println(io,
        "$(dimension(uc))D Unitcell with $(length(uc.basis)) sites and $(length(uc.interactions)) interactions")
end
